 ...
 *   npm run css:copy -- --shade yozakura --chase
 */
...
import { DEFAULT_OPTIONS, CAT_PRESENCE, HOODS, CHASE_SPEEDS } from '../src/lib/options.ts';
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--shade') {
      ...
      if (!CAT_PRESENCE.includes(value)) throw new Error(`--cats must be one of ${CAT_PRESENCE.join(', ')}`);
      options.cats = value;
      fromFile = false;
      i++;
    } else if (arg === '--chase') {
      ...
      if (!HOODS.includes(value)) throw new Error(`--hood must be one of ${HOODS.join(', ')}`);
      options.hood = value;
      fromFile = false;
      i++;
    } else if (arg === '--chase-speed') {
      if (!CHASE_SPEEDS.includes(value)) throw new Error(`--chase-speed must be one of ${CHASE_SPEEDS.join(', ')}`);
      options.chaseSpeed = value;
      fromFile = false;
      i++;
    }
  }
...
function main() {
  ...
  if (fromFile) {
    ...
    css = compose(options);
    source = `composed (${options.shade}, trees=${options.cats}, chase=${options.chase})`;
  }
