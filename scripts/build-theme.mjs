 ...
 * defaults, meaning cats on and the chase off, because a static theme has no
...
const THEME_SHADE = process.env.KONE_SHADE || DEFAULT_OPTIONS.shade;
...
function main() {
  ...
  const manifest = {
    manifestVersion: 1,
    id: 'koneko-static',
    name: `Koneko (${shade.name})`,
    author: 'mdeguzis',
    description: `${shade.description} Costumed cats rest in the corners, behind a translucent interface.`,
    repoUrl: 'https://github.com/mdeguzis/remnote-koneko-theme',
    version: readVersion(),
